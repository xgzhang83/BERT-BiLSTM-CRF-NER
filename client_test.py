...
def ner_test():
    with BertClient(port=5455, port_out=5456, show_server_config=False, check_version=False, check_length=False, mode='NER') as bc:
        start_t = time.perf_counter()
        str = ['把储藏室的灯打开', '关闭警报', '打开窗帘', '总共有多少茶']
        for str1 in str:
            rst = bc.encode([list(str1)], is_tokenized=True)
            # str1 = list(str1)
            # rst = bc.encode([str1], is_tokenized=True)
            print('str: %s, rst: %s' % (str1, rst))
            print(len(rst[0]))
...
def class_test():
    with BertClient(port=5555, port_out=5556, show_server_config=False, check_version=False, check_length=False, mode='CLASS') as bc:
        start_t = time.perf_counter()
        str1 = '把茶仓灯关了'
        str2 = '有多少大滇号的茶'
        str3 = '你好'
        ...
        str5 = '讲个笑话'
        str6 = '几点了'
        rst = bc.encode([str1,str2,str3,str4,str5,str6])
...
#         rst = bc.encode([str, str2, str3])
        print('str:{},{},{},{},{},{}'.format(str1, str2, str3, str4, str5, str6))
        print('rst:', rst)
