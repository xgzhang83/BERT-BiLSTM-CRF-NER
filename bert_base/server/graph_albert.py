...
def optimize_bert_graph(args, logger=None):
    ...
    try:
        ...
        with tf.gfile.GFile(config_fp, 'r') as f:
            bert_config = modeling.AlbertConfig.from_dict(json.load(f))
    ...
    try:
        ...
        with graph.as_default():
            with tf.Session() as sess:
                ...
                bert_config = modeling.AlbertConfig.from_json_file(os.path.join(args.bert_model_dir, 'albert_config.json'))
                from bert_base.train.models import create_albert_model
                (total_loss, logits, trans, pred_ids) = create_albert_model(
    ...
    try:
        ...
        with graph.as_default():
            with tf.Session() as sess:
                ...
                bert_config = modeling.AlbertConfig.from_json_file(os.path.join(args.bert_model_dir, 'albert_config.json'))
                from bert_base.train.models import create_albert_classification_model
                ...
                loss, per_example_loss, logits, probabilities = create_albert_classification_model(bert_config=bert_config, is_training=False, input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, labels=None, num_labels=num_labels)
